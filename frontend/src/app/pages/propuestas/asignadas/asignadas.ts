...
export class PropuestasAsignadasComponent implements OnInit {
  ...
  // Propiedades computadas para estad铆sticas
  ...
  private cargarPropuestasAsignadas(): void {
    ...
    if (!profesorRut) {
      this.error = 'No se encontr贸 el RUT del profesor';
      this.loading = false;
      return;
    }
  ...
  obtenerClaseEstado(estado: string): string {
    ...
    if (estadoLower === 'en_revision' || estadoLower === 'en revisi贸n') return 'estado-en-revision';
  ...
  volver() {
    // Usar history.back() para volver a la p醙ina anterior sin activar guards
    window.history.back();
  }
